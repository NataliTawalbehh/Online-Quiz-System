...
export default class UpdateQuizzes implements FuncAsync<DataObject, Quiz[]> {
  async executeAsync(options?: DataObject): Promise<Quiz[]> {
    try {
      if (!options || !Array.isArray(options.quizzes)) {
        console.error('No quizzes provided to update or quizzes is not an array.');
        return [];
      }

      LocalStorage.set('quizzes', options.quizzes);

      console.log('Quizzes updated successfully:', options.quizzes);
